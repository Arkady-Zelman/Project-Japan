...
def _load_backtest_audit_user(backtest_id: UUID) -> UUID | None:
    """Return the owning user for compute_runs RLS, or None for demo rows."""
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            "select user_id::text from backtests where id = %s",
            (str(backtest_id),),
        )
        row = cur.fetchone()
...
def run_backtest(
    ...
    *,
    ...
    audit_user_id = _load_backtest_audit_user(backtest_id)
    with compute_run("backtest", user_id=audit_user_id) as run:
        run.set_input({
