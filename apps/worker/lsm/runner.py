...
def _load_forecast_paths(
    ...
    for path_id, slot_start, price_kwh in rows:
        ...
        if path_idx < 0 or path_idx >= n_paths:
            raise RuntimeError(
                f"forecast_paths contains path_id={path_idx} outside expected "
                f"range [0, {n_paths - 1}] for run {forecast_run_id}"
            )
        if slot_start not in slot_set:
            slot_set[slot_start] = len(slot_set)
        t_idx = slot_set[slot_start]
        if t_idx >= horizon_slots:
            raise RuntimeError(
                f"forecast_paths contains more than {horizon_slots} distinct "
                f"slots for run {forecast_run_id}"
            )
        paths_kwh[path_idx, t_idx] = float(price_kwh)
    slot_starts = sorted(slot_set.keys())
    if len(slot_starts) != horizon_slots:
        raise RuntimeError(
            f"forecast_paths for run {forecast_run_id} has {len(slot_starts)} "
            f"distinct slots, expected {horizon_slots}"
        )
