...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Keep path rows only for the latest N forecast_runs per area.
    ...
    Keep `forecast_runs` rows themselves: `valuations.forecast_run_id`
    references them without cascade, and those metadata rows are useful for
    audit/history even after their bulky child paths are gone.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify old runs whose path rows can be pruned. Runs feeding
        # queued/running valuations are excluded so in-flight LSM jobs never
        # lose their forecast inputs midway through computation.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        # Delete child rows directly. Do not delete forecast_runs parents:
        # completed valuations and audit flows may still reference them.
        deleted = 0
...
def prune_old_data() -> dict:
    ...
    Historical market/fundamental tables are canonical inputs for training,
    calibration, stack backtests, and public historical charts. The only
    disposable high-volume rows are generated forecast path children, so route
    legacy/on-demand callers through the safe path-pruner.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only forecast_paths are disposable; historical
    # actuals, regimes, and stack outputs feed charts, calibration, and
    # backtests and must remain canonical.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    """Nightly retention sweep for bulky generated forecast path rows."""
