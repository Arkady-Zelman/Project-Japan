...
def _prune_forecast_paths_impl(retain_latest_per_area: int = 2) -> dict:
    ...
    pruning, that grows ~432K/day on the Supabase free tier (500 MB). The
    forecast_runs parent rows are intentionally preserved because valuations
    reference them; only generated path payloads are disposable.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify old run payloads to purge, excluding any run that may be
        # consumed by an in-flight valuation.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        # Delete one run's path payloads at a time. Per-run commits keep WAL
        # small on a near-full disk while preserving forecast_runs metadata.
        deleted = 0
...
def prune_old_data() -> dict:
    """Backward-compatible disk-recovery entrypoint.

    Canonical market, stack, and regime tables are inputs for the dashboard,
    VLSTM training/inference, regime inference, and backtests. Nightly
    retention must therefore only prune generated forecast path payloads.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only generated forecast path payloads are
    # disposable; market, stack, and regime tables feed later model jobs.
    try:
        prune_nightly.spawn()
