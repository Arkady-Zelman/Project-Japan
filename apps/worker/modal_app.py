...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Keep paths only for the latest N forecast_runs per area.
...
def _prune_forecast_paths_impl(retain_latest_per_area: int = 2) -> dict:
    """Delete old forecast_paths while preserving forecast_runs metadata.

    `valuations.forecast_run_id` references forecast_runs without cascade, and
    operators need historical run metadata for audit/debugging. Only the large
    child table is pruned; queued/running valuation runs keep their paths.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify runs whose paths are eligible for pruning. Preserve parent
        # forecast_runs rows and any run still feeding an in-flight valuation.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        prune_run_ids = [r[0] for r in cur.fetchall()]
        out["runs_eligible_for_path_prune"] = len(prune_run_ids)
...
def prune_old_data() -> dict:
    """Compatibility wrapper for the old operator command.

    Historical market/stack/regime tables are canonical project data and are
    no longer pruned here. Keep the name for existing Modal runbooks, but route
    it to the safe forecast-path retention helper.
    """
    out = _prune_forecast_paths_impl(retain_latest_per_area=2)
    print(f"prune_old_data forecast-path result: {out}")
    return out
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only generated forecast_paths are pruned; the
    # market/stack/regime tables are canonical history and must not be deleted
    # by a cron that runs immediately after the daily stack build.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    """Nightly retention sweep for bulky generated forecast paths only."""
