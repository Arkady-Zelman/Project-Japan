...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Keep forecast path rows only for the latest N forecast_runs per area.
    ...
    Do not delete forecast_runs rows here: valuations.forecast_run_id has no
    cascade and completed valuation/backtest audit trails still need the run
    metadata. Queued/running valuations also keep their path rows until the LSM
    job finishes consuming them.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify runs whose heavy child rows can be pruned. We preserve the
        # latest N per area plus any run still referenced by an in-flight
        # valuation; deleting paths for those would make the LSM job fail.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        # Delete path rows one run at a time. Keeping the parent forecast_runs
        # rows avoids breaking valuations.forecast_run_id and preserves audit
        # metadata while reclaiming the large child table.
        ...
        for run_id in purge_ids:
            ...
            path_rows_deleted += cur.rowcount or 0
            conn.commit()
        out["path_rows_deleted"] = path_rows_deleted
        out["forecast_runs_deleted"] = 0
...
@app.function(image=base_image, cpu=2.0, timeout=3600, secrets=_secrets)
def prune_old_data(retain_latest_forecast_runs_per_area: int = 2) -> dict:
    """Compatibility entry point for retention.

    Canonical market, stack, and regime tables are inputs to VLSTM inference,
    MRS calibration, backtests, and dashboard history. Nightly retention must
    not delete or truncate them. The generated forecast_paths child table is
    the bounded high-volume table, so the old operator command now routes to
    the safe forecast-path pruner.
    """
    out = prune_forecast_paths.local(retain_latest_forecast_runs_per_area)
    out["canonical_tables_preserved"] = [
        "jepx_spot_prices",
        "demand_actuals",
        "generation_mix_actuals",
        "weather_obs",
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only prunes generated forecast path rows; the
    # historical market, regime, and stack tables are model inputs and must be
    # preserved for VLSTM lookbacks, MRS calibration, and backtests.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    """Nightly retention sweep for generated forecast path rows."""
