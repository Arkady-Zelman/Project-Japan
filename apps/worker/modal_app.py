...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Keep only path rows for the latest N forecast runs per area.
    ...
    Retention must preserve `forecast_runs` metadata: `valuations.forecast_run_id`
    references it without cascade, and completed valuations/backtests may still
    need the run provenance. We prune only the heavy `forecast_paths` child rows,
    and skip runs attached to queued/running valuations so in-flight LSM jobs do
    not lose their price inputs.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify old runs whose heavy child rows can be pruned. Preserve the
        # parent forecast_runs rows for valuation provenance and FK integrity.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        # Delete child rows in bounded batches. We intentionally do not delete
        # forecast_runs: valuations have an FK to that table without cascade.
        paths_deleted = 0
        for run_id in purge_ids:
            while True:
                ...
                if n < 50000:
                    break
        out["forecast_paths_deleted"] = paths_deleted
...
def prune_old_data() -> dict:
    """Compatibility wrapper for the safe retention sweep.

    Historical market/fundamental tables are canonical product data and feed
    dashboard history, stack overlays, regime inference, VLSTM training, and
    backtests. Nightly retention is therefore limited to generated forecast
    path child rows, which are the high-volume reproducible artifact.
    """
    out = prune_forecast_paths.local()
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Prunes only generated forecast path rows while
    # preserving canonical historical market/fundamental tables and
...
def prune_nightly() -> dict:
    """Nightly retention sweep for generated forecast path child rows."""
