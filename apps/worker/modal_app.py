...
def _prune_forecast_paths_impl(retain_latest_per_area: int = 2) -> dict:
    ...
    Delete only child `forecast_paths` rows. `forecast_runs` is lightweight
    metadata and is referenced by `valuations.forecast_run_id`, so deleting
    parent rows either fails on FK constraints or destroys valuation lineage.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify runs whose path payloads can be purged. Keep the latest N
        # runs per area and any run used by an in-flight valuation.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        out["runs_eligible_for_path_purge"] = len(purge_ids)

        # Delete one forecast_run's path rows at a time. Per-run commits keep
        # WAL small on a near-full disk while preserving forecast_runs rows.
        path_rows_deleted = 0
...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Modal entrypoint for forecast path retention."""
...
def prune_old_data() -> dict:
    ...
    Historical market, stack, weather, and regime tables are canonical inputs
    for training, calibration, backtests, and dashboard drilldowns. Nightly
    retention must never delete or truncate them. The only high-volume
    generated payload safe to prune automatically is `forecast_paths`.
    """
...
def stack_run_daily() -> dict:
    ...
    # to prune automatically; historical market/stack/regime tables are
    # canonical inputs for training, calibration, and backtests.
    try:
        prune_nightly.spawn()
