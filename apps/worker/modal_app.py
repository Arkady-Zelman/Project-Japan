...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    ...
    This intentionally preserves the parent `forecast_runs` rows. Valuations
    reference them for audit/history, and deleting a parent run can either fail
    on the FK or erase the only metadata explaining a completed valuation.
    """
...
def _prune_forecast_paths_impl(retain_latest_per_area: int = 2) -> dict:
    """Implementation shared by explicit and nightly retention entry points."""
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify stale runs whose large child path rows may be purged. Keep
        # path rows for queued/running valuations so an in-flight LSM job never
        # loses its forecast input mid-run.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        out["stale_runs_eligible"] = int(stale_row[0]) if stale_row else 0
        ...
        while True:
            cur.execute(
                ...
                where ctid in (
                  ...
                  ) stale on stale.id = fp.forecast_run_id
                  limit 50000
                )
                """,
                (retain_latest_per_area,),
...
def prune_old_data() -> dict:
    """Backward-compatible retention entry point.

    Historical market, weather, regime, and stack tables are canonical inputs
    for training, backtests, validation, and the AI analyst. Nightly retention
    must not delete or truncate them; only generated forecast path rows are
    disposable once newer runs exist.
    """
    out = _prune_forecast_paths_impl(retain_latest_per_area=2)
    out["canonical_history_preserved"] = True
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep for generated forecast path rows only. Canonical
    # market/stack/regime history must be preserved for training, backtests,
    # validation, and agent queries.
    try:
        prune_nightly.spawn()
