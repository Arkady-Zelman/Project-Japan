...
def _prune_forecast_paths_impl(retain_latest_per_area: int = 2) -> dict:
    """Keep forecast path storage bounded without deleting canonical history.

    `forecast_runs` is intentionally preserved: `valuations.forecast_run_id`
    references it without cascade, and the run metadata is useful even after
    generated paths age out. Paths for queued/running valuations are protected
    so an in-flight LSM job cannot lose its input matrix mid-run.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify runs whose generated paths may be purged. Keep the latest N
        # per area, and keep any run currently feeding a queued/running LSM job.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        # Delete generated child rows only. Parent forecast_runs rows are
        # referenced by valuations and should remain as durable metadata.
        deleted_paths = 0
    ...
    print(f"prune_forecast_paths result: {out}")
    return out
...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """On-demand forecast-path retention entry point."""
...
def prune_old_data() -> dict:
    """Compatibility entry point for the old retention command.

    Historical market, stack, and regime tables are canonical inputs for
    training, backtests, and dashboard drill-downs, so this path now prunes
    only generated forecast path rows.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep for generated forecast paths only. Canonical
    # market/stack/regime history must remain available for VLSTM features,
    # MRS residuals, backtests, and dashboard drill-downs.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    ...
    Spawned from stack_run_daily after the stack build; safe because it never
    mutates the stack, market actuals, or regime history tables.
    """
    return _prune_forecast_paths_impl(retain_latest_per_area=2)
