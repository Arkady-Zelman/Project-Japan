...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Keep recent forecast path payloads without deleting run metadata.

    1000 paths x 48 slots x 9 areas = 432K rows per inference. The bulky
    child rows are safe to prune, but `forecast_runs` is referenced by
    valuations and is useful run metadata, so this function never deletes
    parent rows. Paths for queued/running valuations are also protected so an
    in-flight LSM job cannot lose its input matrix.
    """
    from common.db import connect

    retain = max(int(retain_latest_per_area), 1)
    out: dict = {}
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify runs whose bulky child paths can be purged: not among the
        # latest N per area and not feeding an in-flight valuation.
        cur.execute(
            """
            with ranked as (
              select id,
...
def prune_old_data() -> dict:
    """Backward-compatible entry point for the disk-pressure prune.

    Historical market, stack, and regime tables are canonical inputs for
    model calibration and backtests; pruning them caused silent data loss.
    Keep this operator command, but route it to the narrow forecast-path
    retention routine where the row volume actually grows daily.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only prunes old forecast path payload rows;
    # historical market/stack/regime tables are canonical model inputs.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    ...
    Keep it narrow: forecast path payload rows are high-volume generated data,
    while historical actuals, stack outputs, and regime states are inputs to
    model calibration, backtests, and dashboard history.
    """
    return prune_forecast_paths.local(retain_latest_per_area=2)
