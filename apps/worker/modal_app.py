...
def prune_old_data() -> dict:
    ...
    Canonical market/fundamental history is an input to backtests, model
    training, MRS calibration, and dashboard drilldowns. Nightly retention must
    not delete those tables; only generated forecast path rows are bounded.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only forecast paths are generated high-volume
    # rows safe to bound; canonical market/fundamental history feeds backtests,
    # training, MRS calibration, and dashboard drilldowns.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    ...
    Keep only the latest generated forecast runs per area. Do not prune
    canonical historical tables; downstream analytics require multi-year data.
    """
