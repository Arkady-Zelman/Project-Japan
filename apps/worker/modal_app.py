...
def _prune_forecast_paths_impl(retain_latest_per_area: int = 2) -> dict:
    ...
    Keep forecast_runs metadata so valuation.forecast_run_id references remain
    valid, and keep paths for queued/running valuations so in-flight LSM jobs do
    not lose their input matrix.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify forecast path payloads to purge. We preserve the parent
        # forecast_runs rows for audit/history and any run referenced by a
        # queued/running valuation.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        # Delete path payloads one run at a time. Per-run commit keeps WAL small
        # on a near-full disk without invalidating forecast_run_id references.
        deleted_paths = 0
...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Modal entry point for forecast path retention."""
...
def prune_old_data() -> dict:
    ...
    Historical market, stack, and regime tables are canonical inputs for MRS,
    VLSTM, backtests, and dashboard drilldowns. Nightly retention must therefore
    only trim generated forecast path payloads; operators can still run manual
    VACUUM helpers when they need to reclaim already-dead pages.
    """
    out = _prune_forecast_paths_impl(retain_latest_per_area=2)
    out["canonical_history_pruned"] = False
...
def stack_run_daily() -> dict:
    ...
    # canonical market/stack/regime history is required by downstream models.
    try:
        prune_nightly.spawn()
