...
def prune_old_data() -> dict:
    """Prune bounded generated forecast data only.

    Historical market data, stack curves/clearing prices, and regime states
    are canonical inputs for dashboards, VLSTM inference, and MRS updates.
    Nightly retention must not delete or truncate them.
    """
...
def stack_run_daily() -> dict:
    ...
    # canonical market, stack, and regime tables feed inference/backtests.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    """Nightly forecast retention sweep spawned from stack_run_daily."""
