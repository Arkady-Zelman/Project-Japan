...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Keep path rows only for the latest N forecast runs per area.
    ...
    Preserve forecast_runs parent rows: valuations.forecast_run_id references
    them, and old run metadata is small. Queued/running valuations keep their
    forecast_paths until the LSM worker finishes reading them.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify old path sets to purge. Parent forecast_runs are retained
        # because valuation rows can reference them without ON DELETE CASCADE.
        cur.execute(
            """
            with ranked as (
              select id,
            ...
            from ranked
            left join active_valuations using (id)
            where ranked.rn > %s
              and active_valuations.id is null
            """,
            (retain_latest_per_area,),
        )
        purge_ids = [r[0] for r in cur.fetchall()]
        out["runs_with_paths_to_purge"] = len(purge_ids)

        # Delete child rows one forecast_run_id at a time. Per-run commits keep
        # WAL small on a near-full disk while leaving forecast_runs metadata.
        deleted = 0
...
def prune_old_data() -> dict:
    ...
    data used by dashboards, model calibration, and backtests. Retention is
    therefore limited to generated forecast_paths child rows, which can be
    regenerated by the next forecast inference and dominate daily growth.
    """
    out = prune_forecast_paths.local()
    out["retention_scope"] = "forecast_paths_only"
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Keep only generated forecast path child rows
    # bounded; canonical market/stack/regime history is preserved.
    try:
        prune_nightly.spawn()
