...
def prune_old_data() -> dict:
    ...
    Historical market, stack, and regime tables are product data: dashboard
    charts, VLSTM features, MRS calibration, and backtests all depend on their
    continuity. Only forecast paths are generated bulk output with safe
    retention semantics because deleting old forecast_runs cascades to paths
    while preserving the latest runs per area.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Prunes generated forecast paths only; canonical
    # market, regime, and stack tables are preserved for analytics/backtests.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    """Nightly generated-output retention; preserves canonical tables."""
