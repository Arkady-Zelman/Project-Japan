...
def _prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Keep path payloads for the latest N forecast runs per area.
    ...
    ``forecast_runs`` rows are deliberately preserved: completed valuations
    reference them without ``ON DELETE CASCADE``. Path payloads for queued or
    running valuations are also retained so a concurrent LSM job cannot lose
    its inputs.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify old path payloads that are safe to prune. The parent run
        # metadata stays in place because valuations.forecast_run_id has a
        # non-cascading foreign key to forecast_runs.
        cur.execute(
            """
            with ranked as (
              select fr.id,
                     row_number() over (
                       partition by fr.area_id
                       order by fr.forecast_origin desc, fr.created_at desc, fr.id desc
                     ) as rn
        ...
        # Delete child payloads one run at a time. Per-run commits keep WAL
        # bounded on a near-full database and preserve forecast run metadata.
        deleted_paths = 0
        for run_id in prune_ids:
            ...
            conn.commit()
        out["forecast_path_rows_deleted"] = deleted_paths
...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Operator entry point for generated forecast-path retention."""
    return _prune_forecast_paths(retain_latest_per_area)
...
def prune_old_data() -> dict:
    """Backward-compatible safe retention entry point.

    Market actuals, stack curves, and regime states are canonical historical
    inputs for training and backtests; scheduled retention must never delete
    them. Only generated forecast-path payloads are bounded.
    """
    return _prune_forecast_paths()
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep bounds generated forecast-path payloads. The
    # canonical historical inputs are retained for training and backtests.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    """Nightly retention sweep for generated forecast-path payloads."""
    return _prune_forecast_paths()
