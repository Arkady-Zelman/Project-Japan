...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Keep forecast-path rows only for the latest N forecast runs per area.
    ...
    Keep forecast_runs parent rows: valuations.forecast_run_id references them,
    and completed valuations do not need their full path matrix after decisions
    are persisted. Queued/running valuations still need paths, so exclude those
    runs from pruning.
    """
    ...
    out: dict = {}
    with compute_run("forecast_path_retention") as run:
        with connect() as conn, conn.cursor() as cur:
            ...
            # Identify runs whose child path rows can be pruned. Parent rows
            # remain, so historical valuations keep valid forecast_run_id FKs.
            cur.execute(
                """
                with ranked as (
                  select id,
                ...
                select r.id::text
                  from ranked r
                  left join active_valuations a on a.id = r.id
                 where r.rn > %s
                   and a.id is null
                """,
            ...
            # Flush any open transaction so we can flip autocommit — VACUUM
...
def prune_old_data() -> dict:
    ...
    Historical market, stack, and regime rows are canonical inputs for VLSTM,
    MRS calibration, backtests, and dashboard drilldowns. Do not delete or
    truncate them as part of nightly retention; bound generated forecast paths
    instead.
    """
    out = prune_forecast_paths.local()
    out["notes"] = "canonical history preserved; only forecast_paths were pruned"
    return out
...
def stack_run_daily() -> dict:
    ...
    # market/stack/regime history feeds VLSTM, MRS, backtests, and dashboard
    # drilldowns, so it must not be pruned from this cron.
    try:
        prune_nightly.spawn()
