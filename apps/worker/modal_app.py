...
def _prune_forecast_paths_impl(retain_latest_per_area: int = 2) -> dict:
    ...
    Keep `forecast_runs` metadata for audit/history, delete only child
    `forecast_paths` rows for runs older than the latest N per area, and keep
    paths referenced by queued/running valuations so in-flight LSM jobs do not
    lose their input matrix.
    """
    from common.db import connect

    retain = max(1, int(retain_latest_per_area))
    out: dict = {}
    with connect() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            ...
            using purge_runs p
            where fp.forecast_run_id = p.id
            """,
            (retain,),
        )
        out["forecast_paths_deleted"] = cur.rowcount or 0
        conn.commit()
...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Keep only the latest N runs' generated `forecast_paths` per area.
    ...
    nightly (or on-demand when disk fills) to keep the generated path table
    bounded while preserving canonical history and `forecast_runs` metadata.
    """
...
def prune_old_data() -> dict:
    """Compatibility wrapper for the old on-demand retention entry point.

    Historical market, stack, regime, and weather tables are canonical inputs
    for backtests, calibration, and feature generation. They must not be
    pruned by the nightly disk guard; only generated forecast path rows are
    safe to trim.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly generated-data retention. Canonical market/stack/regime history
    # feeds calibration, feature generation, and backtests, so the disk guard
    # only prunes old forecast_paths rows.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    """Nightly generated-data retention sweep.

    Spawned from stack_run_daily. It intentionally avoids canonical market,
    stack, regime, and weather history; only generated forecast_paths rows are
    trimmed.
    """
