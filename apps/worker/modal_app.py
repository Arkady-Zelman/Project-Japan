...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Keep path rows for the latest N forecast runs per area.
    ...
    Keep every ``forecast_runs`` parent row: completed valuations reference
    those rows without ``ON DELETE CASCADE``. Also keep path rows needed by
    queued/running valuations so retention cannot invalidate active work.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Old runs are metadata and may be referenced by completed valuations,
        # so prune only their large child rows. In-flight valuations still need
        # the paths and are excluded even when their run is old.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        # Delete one run's child rows at a time. Per-run commits keep WAL
        # bounded on a near-full database while parent metadata remains intact.
        paths_deleted = 0
        runs_pruned = 0
...
def prune_old_data() -> dict:
    """Backward-compatible on-demand alias for safe forecast-path retention.

    Canonical market, weather, stack, and regime history feeds model training,
    backtests, and historical UI views and must not be removed by retention.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Forecast paths dominate ongoing growth; retain
    # only the newest path sets while preserving canonical historical inputs.
    try:
        prune_nightly.spawn()
...
def regime_infer_daily() -> dict:
    ...
    init_sentry()
    today = datetime.now(tz=UTC).date()
    # JanczuraWeronMRS requires at least 200 residuals. Fourteen days provides
    # enough half-hourly observations even with ordinary missing input slots.
