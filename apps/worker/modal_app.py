...
def prune_old_data() -> dict:
    """Bound generated forecast paths without deleting canonical history.

    Historical price, demand, generation, weather, stack, and regime rows are
    inputs for model retraining, backtests, and dashboard history. Nightly
    retention must only prune derived forecast path runs, which are regenerated
    daily and dominate incremental row growth.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only prunes generated forecast path runs; the
    # canonical historical tables remain available for model training/backtests.
    try:
        prune_nightly.spawn()
