...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    ...
    Keep the parent forecast_runs rows: valuations reference them, and they
    are small enough to retain as metadata. Purge only forecast_paths for old
    runs, excluding any run still used by queued/running valuations.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify old runs whose heavy path rows can be purged. Parent
        # forecast_runs rows are retained because valuations.forecast_run_id
        # references them without ON DELETE CASCADE.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        # Delete one run's paths at a time. Per-run commit keeps WAL small on
        # a near-full disk while preserving forecast_runs metadata.
        deleted = 0
...
def prune_old_data() -> dict:
    ...
    Historical market, stack, and regime tables are canonical model inputs.
    Nightly retention must not delete or truncate them; only generated VLSTM
    forecast path rows are safe to bound automatically.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Bounds generated VLSTM path rows without
    # deleting canonical market/stack/regime history.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    """Nightly retention sweep for generated VLSTM path rows only."""
    return prune_forecast_paths.local(retain_latest_per_area=2)
