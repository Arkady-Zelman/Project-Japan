...
def _prune_forecast_paths_impl(retain_latest_per_area: int = 2) -> dict:
    """Keep only latest forecast path payloads; preserve canonical history.
    ...
    This intentionally deletes child `forecast_paths` rows only. Parent
    `forecast_runs` rows are audit/model metadata and may be referenced by
    completed valuations; queued/running valuations keep their paths too so
    in-flight LSM jobs cannot lose their input matrix.
    """
    from common.db import connect

    if retain_latest_per_area < 1:
        raise ValueError("retain_latest_per_area must be >= 1")
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            """
            select count(distinct forecast_run_id)
            from valuations
        ...
        out["protected_inflight_runs"] = int(protected_row[0]) if protected_row else 0

        # Identify runs whose bulky child paths can be pruned. Keep latest N
        # per area, plus any run that an in-flight valuation may still load.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        out["runs_selected_for_path_prune"] = len(purge_ids)

        # Delete one run's child rows at a time. Per-run commit keeps WAL
        # bounded on a near-full disk and avoids touching forecast_run metadata.
        paths_deleted = 0
        for run_id in purge_ids:
            cur.execute("delete from forecast_paths where forecast_run_id = %s", (run_id,))
            paths_deleted += cur.rowcount or 0
            conn.commit()
        out["forecast_paths_rows_deleted"] = paths_deleted
...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Modal entrypoint for safe forecast-path retention."""
...
def prune_old_data() -> dict:
    """Compatibility alias for the old retention command.

    Historical market, demand, generation, regime, and stack tables are
    canonical analytics inputs. Disk relief must only prune generated forecast
    path payloads, leaving those source/derived history tables intact.
    """
    out = _prune_forecast_paths_impl(retain_latest_per_area=2)
    print(f"prune_old_data (forecast_paths-only) result: {out}")
    return out
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only generated forecast path payloads are safe
    # to prune; historical actuals, regimes, and stack outputs are canonical
    # analytics inputs for calibration, inference, backtests, and dashboards.
    try:
        prune_nightly.spawn()
