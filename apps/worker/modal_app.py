...
def prune_old_data() -> dict:
    """Bound generated forecast path storage without deleting canonical history.

    The historical market/fundamental/regime tables are product data: backtests,
    calibration, VLSTM training, and historical dashboard views depend on them.
    Only generated forecast runs are safe for automatic nightly retention.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only generated forecast paths are bounded here;
    # canonical history is needed by backtests, model training, and charts.
    try:
        prune_nightly.spawn()
