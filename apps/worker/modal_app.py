...
def prune_old_data() -> dict:
    ...
    Historical actuals, stack curves/clearing prices, and regime states are
    inputs to VLSTM training/inference, MRS calibration, backtests, and the
    dashboard. The only high-volume table generated twice daily by routine
    compute is `forecast_paths`, whose parent rows cascade safely.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only generated forecast paths are pruned; the
    # canonical history and stack outputs feed VLSTM, MRS, backtests, and the
    # dashboard, so deleting them creates data loss and downstream failures.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    """Nightly retention sweep for generated forecast paths only."""
