...
def prune_old_data() -> dict:
    """Compatibility wrapper for safe retention.

    The canonical historical tables are product data and must not be pruned by
    scheduled maintenance. Only generated forecast paths are bounded.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Keep generated forecast paths bounded while
    # preserving canonical historical market, weather, regime, and stack data.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    ...
    Keep only the latest generated forecast runs per area; never delete
    canonical historical actuals, regime states, or stack model outputs.
    """
    return prune_forecast_paths.local(retain_latest_per_area=2)
