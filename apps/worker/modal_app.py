...
def prune_old_data() -> dict:
    ...
    Historical market data, weather observations, regime states, and stack
    outputs are inputs to the dashboard, calibration, and VLSTM lookbacks. The
    only nightly retention-safe rows are generated forecast runs/paths.
    """
...
def stack_run_daily() -> dict:
    ...
    # canonical inputs/derived stack outputs are preserved for downstream
    # calibration, dashboard, and VLSTM lookbacks.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    """Nightly retention sweep for generated forecast runs/paths only."""
