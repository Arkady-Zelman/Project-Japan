...
def _prune_forecast_paths_impl(retain_latest_per_area: int = 2) -> dict:
    """Keep paths only for the latest N forecast runs per area.

    1000 paths × 48 slots × 9 areas × N runs = 432K × N rows. With nothing
    pruning, that grows ~432K/day on the Supabase free tier (500 MB).

    Forecast run rows are intentionally preserved: valuations.forecast_run_id
    references them without ON DELETE CASCADE, and the run metadata is useful
    for audit/history even after the bulky path rows age out.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        # Identify old runs whose bulky child paths can be purged. Do not touch
        # queued/running valuation runs because LSM may still need those paths.
        cur.execute(
            """
            with ranked as (
              select id,
        ...
        out["active_valuation_runs_protected"] = int(protected_row[0]) if protected_row else 0

        # Delete child path rows one run at a time. Per-run commit keeps WAL
        # bounded when the database is near its disk limit.
        deleted_paths = 0
...
def prune_old_data() -> dict:
    """Backward-compatible entry point for retention.

    Historical market, weather, stack, and regime tables are canonical product
    data and must not be trimmed by an automated disk sweep. The only bounded
    high-volume table is forecast_paths.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only forecast_paths is bounded; canonical
    # market/history tables are preserved for models and dashboards.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    ...
    Keep this forecast-path-only. The market, weather, stack, and regime
    history is canonical data used by backtests, model calibration, and the UI.
    """
