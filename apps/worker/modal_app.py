...
def _prune_forecast_paths_impl(retain_latest_per_area: int = 2) -> dict:
    """Keep generated forecast path rows bounded without deleting source data.

    1000 paths × 48 slots × 9 areas × N runs = 432K × N rows. With nothing
    pruning, that grows ~432K per inference on the Supabase free tier. Retain
    the parent `forecast_runs` rows so valuation metadata FKs and audit history
    remain intact; only old child `forecast_paths` rows are disposable.
    """
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            ...
            where status in ('queued', 'running')
              and forecast_run_id is not null
            """
        )
        protected_row = cur.fetchone()
        out["active_valuation_runs_protected"] = (
            int(protected_row[0]) if protected_row else 0
        )
        ...
        cur.execute(
            ...
            select count(distinct fp.forecast_run_id)
            from forecast_paths fp
            join pruneable p on p.id = fp.forecast_run_id
            """,
            (retain_latest_per_area,),
        )
        pruneable_row = cur.fetchone()
        out["runs_with_paths_to_prune"] = int(pruneable_row[0]) if pruneable_row else 0
        ...
        out["forecast_delete_batches"] = batches
        out["forecast_runs_preserved"] = True
...
def prune_forecast_paths(retain_latest_per_area: int = 2) -> dict:
    """Prune old generated forecast path rows, preserving source/history tables."""
...
def prune_old_data() -> dict:
    """Legacy retention entry point.

    Canonical market/model tables are source-of-truth history for stack builds,
    MRS calibration, VLSTM windows, and backtests. The only scheduled/on-demand
    rows safe to discard are generated `forecast_paths` children for old runs.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Only generated forecast path rows are pruned;
    # canonical market/model history stays intact for stack, MRS, VLSTM, and
    # backtest consumers.
    try:
        prune_nightly.spawn()
