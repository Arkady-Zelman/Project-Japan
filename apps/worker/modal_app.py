...
def prune_old_data() -> dict:
    ...
    Historical market, weather, stack, and regime tables are canonical inputs
    for backtests, calibration, and analysis. Automatic retention must only
    remove generated forecast path runs, which are reproducible and otherwise
    grow by hundreds of thousands of rows per forecast.
    """
...
def stack_run_daily() -> dict:
    ...
    # Nightly retention sweep. Forecast paths are generated artifacts and grow
    # by ~432K rows per run; canonical historical inputs must be preserved for
    # backtests, calibration, and AI analysis.
    try:
        prune_nightly.spawn()
...
def prune_nightly() -> dict:
    ...
    Only forecast_runs / forecast_paths are pruned. The market actuals,
    weather, stack, and regime tables are canonical history and must not be
    deleted by automatic retention.
    """
    return prune_forecast_paths.local(retain_latest_per_area=2)
