...
-- ============================================================
-- `region_history` is executable by the public anon role for the dashboard map.
-- Keep the RPC self-limiting even when callers bypass the Next.js route.
-- ============================================================
...
begin
  ...
  from (
    ...
    from unnest(coalesce(p_area_codes, '{}')) as requested(code)
    join areas a on a.code = requested.code
    order by a.code
    limit 10
  ) allowed;
  ...
  if p_metric = 'price' then
    ...
    where a.code = any(v_area_codes)
      ...
      and jsp.slot_start >= now() - (v_days::text || ' days')::interval
    ...
    with gen as (
      ...
      where gma.slot_start >= now() - (v_days::text || ' days')::interval
      group by 1, 2
    )
    select
      gen.day,
    ...
    with day_demand as (
      ...
      from demand_actuals
      where slot_start >= now() - (v_days::text || ' days')::interval
